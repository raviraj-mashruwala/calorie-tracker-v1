...
class CalorieTracker {
    constructor() {
        ...
        this.commonFoods = [
            ...
            {name: "White Rice (cooked)", calories: 206, unit: "cup", servingSize: 1},
            {name: "Chicken Breast (cooked)", calories: 231, unit: "100g", servingSize: 100},
            {name: "Bread (white slice)", calories: 79, unit: "slice", servingSize: 1},
    ...
    loadData() {
        this.data = {
            profiles: JSON.parse(localStorage.getItem('ct_profiles')) || [],
            currentProfileId: localStorage.getItem('ct_currentProfile') || null,
            foodEntries: JSON.parse(localStorage.getItem('ct_foodEntries')) || {},
            exerciseEntries: JSON.parse(localStorage.getItem('ct_exerciseEntries')) || {},
            ingredients: JSON.parse(localStorage.getItem('ct_ingredients')) || [],
            customFoods: JSON.parse(localStorage.getItem('calorieTracker_customFoods')) || []
        };
    }

    // Save data to localStorage
    saveData() {
        localStorage.setItem('ct_profiles', JSON.stringify(this.data.profiles));
        localStorage.setItem('ct_currentProfile', this.data.currentProfileId);
        localStorage.setItem('ct_foodEntries', JSON.stringify(this.data.foodEntries));
        localStorage.setItem('ct_exerciseEntries', JSON.stringify(this.data.exerciseEntries));
        localStorage.setItem('ct_ingredients', JSON.stringify(this.data.ingredients));
        localStorage.setItem('calorieTracker_customFoods', JSON.stringify(this.data.customFoods));
    }
    ...
    isDuplicateFood(name) {
        return this.foodDatabase.some(food => 
    ...
    setupEventListeners() {
        ...
        document.getElementById('quickBuildFoodBtn').addEventListener('click', () => {
            this.showFoodBuilderModal();
        });

        document.getElementById('foodBuilderForm').addEventListener('submit', (e) => {
            e.preventDefault();
            this.saveCustomFood();
        });

        document.getElementById('cancelFoodBuilderBtn').addEventListener('click', () => {
            this.hideModal('foodBuilderModal');
        });

        document.getElementById('addIngredientToFood').addEventListener('click', () => {
        ...
        document.getElementById('quickAddExerciseBtn').addEventListener('click', () => {
            this.showExerciseModal();
        });

        document.getElementById('exerciseForm').addEventListener('submit', (e) => {
            e.preventDefault();
            this.saveExerciseEntry();
        });
    ...
    setupAutocomplete() {
        ...
        document.getElementById('foodNameInput').addEventListener('keydown', (e) => {
            const suggestionItems = suggestions.querySelectorAll('.autocomplete-item');
            ...
            if (e.key === 'ArrowDown') {
                ...
                if (selectedIndex >= 0 && suggestionItems[selectedIndex]) {
                    this.selectSuggestion(JSON.parse(suggestionItems[selectedIndex].dataset.food));
                }
    ...
    showSuggestions(query) {
        ...
        const matches = this.foodDatabase.filter(food =>
    ...
    showIngredientModal(ingredient = null) {
        ...
        if (ingredient) {
            ...
            document.querySelector(`input[name="baseUnit"][value="${baseUnit}"]`).checked = true;
            this.updateBaseUnitDisplay(baseUnit);
            
            // Set optional nutritional values
            document.getElementById('ingredientProtein').value = ingredient.protein || '';
            ...
            form.reset();
            document.querySelector('input[name="baseUnit"][value="100g"]').checked = true;
    ...
    saveIngredient() {
        ...
        const baseUnit = document.querySelector('input[name="baseUnit"]:checked').value;
        ...
        const ingredientData = {
            ...
            baseUnit: baseUnit,
            protein: parseFloat(document.getElementById('ingredientProtein').value) || null,
            carbs: parseFloat(document.getElementById('ingredientCarbs').value) || null,
            fat: parseFloat(document.getElementById('ingredientFat').value) || null,
            createdAt: this.editingId ? this.data.ingredients.find(i => i.id === this.editingId)?.createdAt || new Date().toISOString() : new Date().toISOString()
        };

        if (this.editingId) {
            const index = this.data.ingredients.findIndex(i => i.id === this.editingId);
            if (index !== -1) {
    ...
    deleteIngredient(ingredientId) {
        ...
        this.data.ingredients = this.data.ingredients.filter(i => i.id !== ingredientId);
        this.saveData();
        this.renderIngredients();
        ...
        this.data.ingredients.forEach(ingredient => {
            ...
            div.innerHTML = `
                ...
                <div class="ingredient-info">
                    ...
                    <span class="ingredient-unit-badge">${ingredient.baseUnit}</span>
                </div>
                <div class="ingredient-actions">
                    <button class="btn btn--sm btn--secondary" onclick="app.showIngredientModal(${JSON.stringify(ingredient).replace(/"/g, '&quot;')})">Edit</button>
                    <button class="btn btn--sm delete-btn" onclick="app.deleteIngredient('${ingredient.id}')">Delete</button>
                </div>
            `;
            container.appendChild(div);
        });
    ...
    setupFoodBuilderEventListeners() {
        ...
        ingredientAmount.parentNode.replaceChild(newIngredientAmount, ingredientAmount);
        ...
        document.getElementById('ingredientAmount').addEventListener('input', () => {
            this.updateIngredientCalculation();
        });
    }
    ...
    updateIngredientCalculation() {
        ...
        const caloriesDisplay = document.getElementById('ingredientCaloriesDisplay');
    ...
    removeIngredientFromFood(selectedId) {
        this.selectedIngredients = this.selectedIngredients.filter(item => item.id !== selectedId);
        this.updateSelectedIngredientsList();
    }
            ...
            this.selectedIngredients.forEach(item => {
                ...
                div.innerHTML = `
                    <div class="selected-ingredient-info">
                        <div class="selected-ingredient-name">${item.ingredient.name}</div>
                        <div class="selected-ingredient-amount">${item.amount}${item.unit} = ${item.calories.toFixed(1)} kcal</div>
                    </div>
                    <button class="btn btn--sm delete-btn" onclick="app.removeIngredientFromFood('${item.id}')">Remove</button>
                `;
                container.appendChild(div);
            });
        }
    ...
    saveCustomFood() {
        ...
        const customFoodName = document.getElementById('customFoodName').value.trim();
        const mealType = document.getElementById('builderMealType').value;
        const totalCalories = this.selectedIngredients.reduce((sum, item) => sum + item.calories, 0);
    ...
    loadCurrentProfile() {
        if (this.data.currentProfileId) {
            this.currentProfile = this.data.profiles.find(p => p.id === this.data.currentProfileId);
        }
    ...
    saveProfile() {
        ...
        if (this.editingId) {
            const index = this.data.profiles.findIndex(p => p.id === this.editingId);
            if (index !== -1) {
    ...
    populateExerciseSelect() {
        ...
        exerciseSelect.innerHTML = '<option value="">Select exercise</option><option value="custom">Custom exercise</option>';
    ...
    saveFoodEntry() {
        ...
        if (!this.isDuplicateFood(foodName)) {
            const shouldSave = confirm(`"${foodName}" is not in our database. Would you like to save it for future use?`);
            if (shouldSave) {
                const customFood = {
                    id: 'custom_' + Date.now(),
                    name: foodName,
    ...
    deleteFoodEntry(entryId) {
        ...
        const profileId = this.currentProfile.id;
        if (this.data.foodEntries[profileId] && this.data.foodEntries[profileId][this.currentDate]) {
            this.data.foodEntries[profileId][this.currentDate] = this.data.foodEntries[profileId][this.currentDate].filter(entry => entry.id !== entryId);
            this.saveData();
            this.renderFoodEntries();
            this.updateDashboard();
    ...
    renderFoodEntries() {
        ...
        mealTypes.forEach(mealType => {
            ...
            if (this.currentProfile && this.data.foodEntries[this.currentProfile.id] && this.data.foodEntries[this.currentProfile.id][this.currentDate]) {
                const mealName = mealType.charAt(0).toUpperCase() + mealType.slice(1);
                const entries = this.data.foodEntries[this.currentProfile.id][this.currentDate]
                    .filter(entry => entry.meal === mealName || (mealType === 'snack' && entry.meal === 'Snack'));
                ...
                entries.forEach(entry => {
                    ...
                    if (entry.isCustomFood && entry.ingredients) {
                        detailsText = 'Custom recipe: ' + entry.ingredients.map(ing => `${ing.amount}${ing.unit} ${ing.name}`).join(', ');
                    } else {
                        detailsText = `${entry.quantity} ${entry.unit || ''} × ${entry.caloriesPerUnit} kcal`.trim();
                    }
            ...
            if (mealTotal === 0) {
                const div = document.createElement('div');
                div.className = 'empty-state';
                div.textContent = `No ${mealType === 'snack' ? 'snacks' : mealType} items logged`;
                container.appendChild(div);
            }
    ...
    saveExerciseEntry() {
        ...
        if (manualCalories) {
            ...
            caloriesBurned = Math.round(met * this.currentProfile.weight * (duration / 60));
        } else {
            // Default estimation
    ...
    deleteExerciseEntry(entryId) {
        ...
        const profileId = this.currentProfile.id;
        if (this.data.exerciseEntries[profileId] && this.data.exerciseEntries[profileId][this.currentDate]) {
            this.data.exerciseEntries[profileId][this.currentDate] = this.data.exerciseEntries[profileId][this.currentDate].filter(entry => entry.id !== entryId);
            this.saveData();
            this.renderExerciseEntries();
            this.updateDashboard();
    ...
    renderExerciseEntries() {
        ...
        if (this.currentProfile && this.data.exerciseEntries[this.currentProfile.id] && this.data.exerciseEntries[this.currentProfile.id][this.currentDate]) {
            const entries = this.data.exerciseEntries[this.currentProfile.id][this.currentDate];
            ...
            div.innerHTML = `
                ...
                <div class="profile-details">
                    <div class="profile-detail">
                        ...
                        <span class="profile-detail-value">${profile.age} years</span>
                    </div>
                    <div class="profile-detail">
                ...
                <div class="profile-actions">
                    ${!isActive ? `<button class="btn btn--sm btn--primary" onclick="app.switchProfile('${profile.id}')">Select</button>` : ''}
                    <button class="btn btn--sm btn--secondary" onclick="app.showProfileModal(${JSON.stringify(profile).replace(/"/g, '&quot;')})">Edit</button>
                    <button class="btn btn--sm delete-btn" onclick="app.deleteProfile('${profile.id}')">Delete</button>
                </div>
            `;
            container.appendChild(div);
        });
    ...
    updateDashboard() {
        ...
        const consumed = this.getTotalCaloriesConsumed(this.currentProfile.id, this.currentDate);
        const burned = this.getTotalCaloriesBurned(this.currentProfile.id, this.currentDate);
    ...
    getTotalCaloriesConsumed(profileId, date) {
        if (!this.data.foodEntries[profileId] || !this.data.foodEntries[profileId][date]) {
            return 0;
        }
        return this.data.foodEntries[profileId][date].reduce((total, entry) => total + entry.totalCalories, 0);
    }

    getTotalCaloriesBurned(profileId, date) {
        if (!this.data.exerciseEntries[profileId] || !this.data.exerciseEntries[profileId][date]) {
            return 0;
        }
        return this.data.exerciseEntries[profileId][date].reduce((total, entry) => total + entry.caloriesBurned, 0);
    }
    ...
    showSummaryTab(summaryType) {
        ...
        document.querySelector(`[data-summary="${summaryType}"]`).classList.add('active');
    ...
    const summaryDate = document.getElementById('summaryDate').value || this.currentDate;
    const consumed = this.getTotalCaloriesConsumed(this.currentProfile.id, summaryDate);
    const burned = this.getTotalCaloriesBurned(this.currentProfile.id, summaryDate);
    const bmr = Math.round(this.calculateBMR(this.currentProfile));
    ...
    updateWeeklyChart() {
		  ...
		  // Build last 7 days
		  for (let i = 6; i >= 0; i--) {
			const d = new Date(this.currentDate || new Date().toISOString().split('T')[0]);
			d.setDate(d.getDate() - i);
			const dateStr = d.toISOString().split('T')[0];

			const consumedVal = this.getTotalCaloriesConsumed(this.currentProfile.id, dateStr);
			const burnedVal = this.getTotalCaloriesBurned(this.currentProfile.id, dateStr);
		  ...
		  const totalNetText = `${totalNet >= 0 ? 'Surplus' : 'Deficit'} ${totalNet} kcal`;
			  ...
			  datasets: [
				{
				  ...
				  borderWidth: 3,
				  fill: false,
				  pointBackgroundColor: netData.map(v => (v >= 0 ? '#22c55e' : '#ef4444')),
				  pointRadius: 3,
				  spanGaps: true
				},
				{
    ...
    updateMonthlyChart() {
	  ...
	  for (let i = 29; i >= 0; i--) {
		...
		const consumedCals = this.getTotalCaloriesConsumed(this.currentProfile.id, dateStr);
		const burnedCals = this.getTotalCaloriesBurned(this.currentProfile.id, dateStr);
